...
<head>
	...
	<link rel="icon" type="image/ico" href="images/iconlogo.ico">
		...
		<div class="feed">
			...
			<div class="container-comunidade">
					...
					<div class="container-comunidade-wraper">
						<?php foreach(\AmDev\Models\UsuariosModel::listarAmigos() as $key => $value){ ?>
			...
			<br/>
				...
				<div class="container-comunidade">
					...
					<?php
							$comunidade = \AmDev\Models\UsuariosModel::listarComunidade();

							foreach ($comunidade as $key => $value) {

								$pdo = \AmDev\MySql::connect();
								$verificarAmizade = $pdo->prepare("SELECT * FROM amizade WHERE (enviou = ? AND recebeu = ? AND status = 1) OR (enviou = ? AND recebeu = ? AND status = 1)");
						...
						<div class="container-comunidade-single">
							...
							<div class="btn-solicitar-amizade">
								<?php
									if(\AmDev\Models\UsuariosModel::existePedidoAmizade($value['id'])){
								?>
