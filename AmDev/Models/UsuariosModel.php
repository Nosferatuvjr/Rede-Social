<?php
    ...
    class UsuariosModel{
        ...
        public static function solicitarAmizade($idPara){
            ...
            $verificaAmizade = $pdo->prepare("SELECT * FROM amizade WHERE (enviou = ? AND recebeu = ?) OR (enviou = ? AND recebeu = ?)");
            ...
            if($verificaAmizade->rowCount() == 1){
                ...
                $insertAmizade = $pdo->prepare("INSERT INTO amizade VALUES (null,?,?,0)");
                if(
        ...
        public static function listarAmizadesPendentes(){
            ...
            $listarAmizadesPendentes = $pdo->prepare("SELECT * FROM amizade WHERE recebeu = ? AND status = 0");
        ...
        public static function atualizarPedidoAmizade($enviou,$status){
            ...
            if($status == 0){
                //recusei o pedido
                $del = $pdo->prepare("DELETE FROM amizade WHERE enviou = ? AND recebeu = ? AND status = 0");
				$del->execute(array($enviou,$_SESSION['id']));
            }else if($status == 1){
                //aceitei o pedido
                $aceitarPedido = $pdo->prepare("UPDATE amizade SET status = 1 WHERE enviou = ? AND recebeu = ?");
        ...
        public static function listarAmigos(){
			...
			$amizades = $pdo->prepare("SELECT * FROM amizade WHERE (enviou = ? AND status = 1) OR (recebeu = ? AND status = 1)");
